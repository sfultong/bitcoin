...
*Setup instructions for a gitian build of BitcoinUnified using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BitcoinUnified
...
- [Building BitcoinUnified](#building-bitcoinunified)
...
Clone the git repositories for bitcoinunified and gitian and then checkout the bitcoinunified version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/sfultong/bitcoin.git
cd bitcoinunified
...
Follow the instructions in [doc/release-process.md](release-process.md) in the bitcoinunified repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building BitcoinUnified
----------------

To build BitcoinUnified (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bitcoinunified repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitcoinunified/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/sfultong/bitcoin
...
For example:
```bash
URL=https://github.com/user/bitcoinunified.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bitcoinunified=${COMMIT} --url bitcoinunified=${URL} ../bitcoinunified/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoinunified=${COMMIT} --url bitcoinunified=${URL} ../bitcoinunified/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitcoinunified=${COMMIT} --url bitcoinunified=${URL} ../bitcoinunified/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcoinunified-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcoinunified-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/bitcoinunified-build.assert
```
...
[bitcoinunified/gitian.sigs](https://github.com/litecoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
